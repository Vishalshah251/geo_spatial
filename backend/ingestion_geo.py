...
def _grid_points_sg(step_deg: float = 0.03) -> List[Tuple[float, float]]:
...
def _load_sample_geoapify() -> List[POI]:
    sample_path = Path("data/geoapify_sample.json")
    ...
    for f in features:
        ...
        poi = POI(
            id=f"geoapify:{props.get('place_id') or props.get('datasource', {}).get('raw', {}).get('id') or sha1_text(name)}",
            name=name,
            lat=float(lat),
            lon=float(lon),
...
def fetch_geoapify_places(
    ...
    if not api_key:
        log("GEOAPIFY_API_KEY not set; using sample data")
        return _load_sample_geoapify()
